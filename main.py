...
class Plugin:
    async def _main(self):
        decky.logger.info("Loading plugin")
    ...
    async def retrieve_http_port(self):
        try:
            decky.logger.info(f"retrieving http_port from {HHD_STATE_PATH_LIST}")
            ...
            for path in HHD_STATE_PATH_LIST:
                ...
                decky.logger.info(f"http_port {port}")
                if port == "default":
                    return DEFAULT_PORT
    ...
    async def retrieve_hhd_token(self):
        try:
            decky.logger.debug(f"retrieving token from {HHD_TOKEN_PATH_LIST}")
            for path in HHD_TOKEN_PATH_LIST:
                if os.path.exists(path):
                    decky.logger.info(f"token path {path}")
                    token = open(path, "r").read()
                    decky.logger.info(f"token {token}")
                    return token
            return False
        except Exception as e:
            decky.logger.error(f"failure retrieving token {e}", exc_info=True)
            return False
    ...
    async def get_settings(self):
        try:
            ...
            decky.logger.error(f"failure retrieving settings {e}", exc_info=True)
            return False
    ...
    async def ota_update(self):
        # trigger ota update
        try:
            decky.logger.info("ota update main.py")
            plugin_update.ota_update()
        except Exception as e:
            decky.logger.error(f"failure ota update {e}", exc_info=True)
